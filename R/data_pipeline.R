...
#' @param target_col Character string. The primary target variable ("OC").
#' @param covariate_cols Character vector. Names of covariate columns.
#' @param shape_path Character string. Relative path to the boundary shapefile.
#'
#' @importFrom stats dist median var complete.cases
...
load_and_validate_data <- function(
  ...
  # Extract projected coordinates to standalone columns
  projected_coords <- sf::st_coordinates(spatial_sf)
  spatial_sf$X <- projected_coords[, 1]
  spatial_sf$Y <- projected_coords[, 2]
